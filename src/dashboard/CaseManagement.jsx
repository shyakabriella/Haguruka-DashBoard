...
export default function CaseManagement() {
  ...
  const handleCaseStatusUpdated = (caseId, newStatus, updatedRawCase = null) => {
    const updatedAt = new Date().toISOString();

    let normalizedUpdatedCase = null;

    if (updatedRawCase && updatedRawCase.id) {
      normalizedUpdatedCase = normalizeCase(updatedRawCase);
    }

    setCases((prev) =>
      prev.map((item) => {
        if (Number(item.id) !== Number(caseId)) return item;

        return normalizedUpdatedCase
    ...
    setSelected((prev) => {
      if (!prev || Number(prev.data?.id) !== Number(caseId)) return prev;

      return {
        ...prev,
        data: normalizedUpdatedCase
          ? normalizedUpdatedCase
          : {
              ...prev.data,
              status: normalizeStatus(newStatus),
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            View victim reports, reporter identity, evidence, exact location,
            map, route, status updates, and follow-up tasks.
          </p>
      ...
      <div className="w-full max-w-6xl bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] overflow-y-auto">
        ...
        {activeTab === "overview" ? (
          <CaseOverviewPanel
            ...
            onCaseStatusUpdated={onCaseStatusUpdated}
          />
        ) : null}
...
function CaseOverviewPanel({ data, onUnauthorized, onCaseStatusUpdated }) {
  return (
    <div className="p-5 space-y-4">
      ...
      <CaseStatusUpdatePanel
        caseData={data}
        onUnauthorized={onUnauthorized}
        onCaseStatusUpdated={onCaseStatusUpdated}
      />

      <CaseActionReasonPanel data={data} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Info
          label="Urgency"
          value={<UrgencyBadge value={data.urgency} raw={data.raw_urgency} />}
        />
        <Info label="Current Status" value={<StatusBadge value={data.status} />} />
      ...
      <div>
        <div className="text-xs font-bold text-slate-700">Case Details</div>
...
function CaseActionReasonPanel({ data }) {
  const isWithdrawn = data.status === "withdrawn";
  const isClosed = data.status === "closed";

  if (!isWithdrawn && !isClosed) {
    return null;
  }

  const reason = isWithdrawn ? data.withdraw_reason : data.closed_reason;
  const actionAt = isWithdrawn ? data.withdrawn_at : data.closed_at;
  const actionByUser = isWithdrawn
    ? data.withdrawn_by_user
    : data.closed_by_user;

  const title = isWithdrawn
    ? "Case Withdrawn - Reason / Motif"
    : "Case Closed - Reason / Motif";

  const wrapperClass = isWithdrawn
    ? "rounded-2xl border border-red-200 bg-red-50 p-4"
    : "rounded-2xl border border-slate-200 bg-slate-50 p-4";

  const titleClass = isWithdrawn
    ? "font-extrabold text-red-900"
    : "font-extrabold text-slate-900";

  const helpClass = isWithdrawn
  ...
  return (
    <div className={wrapperClass}>
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
        <div>
          ...
          <p className={helpClass}>
            This is the Reason / Motif submitted when this case was finalized.
          </p>
        </div>
      ...
      <div className="mt-4 rounded-xl bg-white border border-white/70 p-4">
        <div className="text-xs font-bold text-slate-500">Reason / Motif</div>
        <div className="mt-2 text-sm font-semibold text-slate-900 leading-6 whitespace-pre-wrap">
          {reason?.trim() ? reason : "No reason/motif was provided."}
        </div>
      </div>

      <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Info
          label={isWithdrawn ? "Withdrawn At" : "Closed At"}
          value={formatDateTime(actionAt)}
        />

        <Info
          label={isWithdrawn ? "Withdrawn By" : "Closed By"}
          value={
            actionByUser?.name
              ? `${actionByUser.name}${
                  actionByUser.email ? ` (${actionByUser.email})` : ""
                }`
              : "Unknown"
          }
        />
...
function CaseStatusUpdatePanel({
  caseData,
  onUnauthorized,
  onCaseStatusUpdated,
}) {
  ...
  useEffect(() => {
    setStatus(caseData.status || "submitted");
  }, [caseData.status]);

  const submit = async (e) => {
    e.preventDefault();

    await updateCaseStatusApi({
      caseId: caseData.id,
      status,
      setSaving,
      setError,
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-blue-700 mt-1">
            Change the case progress after reviewing evidence or follow-up
            actions.
          </p>
        ...
        <form
          onSubmit={submit}
          className="w-full lg:max-w-xl grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2"
        >
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value)}
            className="w-full rounded-xl border border-blue-200 px-4 py-2 text-sm bg-white outline-none focus:border-teal-400 focus:ring-4 focus:ring-teal-100"
          >
            <option value="submitted">Submitted</option>
            <option value="pending">Pending</option>
            <option value="under_review">Under Review</option>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-blue-700 hover:bg-blue-800 text-white text-sm font-extrabold disabled:opacity-60"
          >
...
function FollowUpTasksPanel({
  ...
  useEffect(() => {
    fetchTasks();
  }, [caseData.id, status, priority, assignedTo]);
  ...
  const createTask = async (payload) => {
    try {
      ...
      if (payload.status === "in_progress") {
        await updateCaseStatusFromFollowUp("in_progress");
      }
    } catch (err) {
  ...
  const updateCaseStatusFromFollowUp = async (newStatus) => {
    try {
      const response = await fetch(
        `${API_BASE}/victim-reports/${caseData.id}/status`,
        {
          method: "PATCH",
          headers: authHeaders(),
      ...
      if (response.ok && result?.success !== false) {
        onCaseStatusUpdated?.(caseData.id, newStatus, result?.data || null);
      }
    } catch {
      onCaseStatusUpdated?.(caseData.id, newStatus, null);
    }
  };
  ...
  const updateTaskStatus = async (task, newStatus) => {
    try {
      ...
      const updatedTask = normalizeTask(result.data || task);
      ...
      if (newStatus === "done") {
        ...
        const allTasksFinished = nextTasks.every(
          (item) => item.status === "done" || item.status === "cancelled"
        );

        if (allTasksFinished) {
          await updateCaseStatusFromFollowUp("resolved");
        }
      }
      ...
      <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              Follow-up actions can move this case from submitted to in
...
function CreateFollowUpTaskModal({
  caseCode,
  users,
  usersLoading,
  loading,
...
async function updateCaseStatusApi({
  caseId,
  status,
  setSaving,
  setError,
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/victim-reports/${caseId}/status`, {
      method: "PATCH",
      headers: authHeaders(),
    ...
    onCaseStatusUpdated?.(caseId, status, result?.data || null);
    setMessage?.("Case status updated successfully.");
  } catch (err) {
    setError?.(err?.message || "Could not update case status.");
  } finally {
    setSaving?.(false);
  }
}
...
function normalizeCase(item) {
  ...
  return {
    id: item.id,
    code: item.case_code || buildCaseCode(item.id),
    ...
    // Case final action fields: shown to admin when victim withdraws/closes case.
    withdraw_reason:
    ...
    withdrawn_by: item?.withdrawn_by || null,
    withdrawn_by_user: item?.withdrawn_by_user || null,
    ...
    closed_by_user: item?.closed_by_user || null,
...
function normalizeUser(user) {
  const roles = Array.isArray(user?.roles)
    ? user.roles.map((role) => role?.name || role?.slug || role).filter(Boolean)
    : [];
...
function buildFileUrl(url) {
  ...
  if (cleanUrl.startsWith("victim-reports/")) {
    return `${backendRoot}/storage/${cleanUrl}`;
  }
...
function getEvidenceKind(file) {
  ...
  if (type.includes("pdf") || name.endsWith(".pdf") || url.endsWith(".pdf")) {
    return "pdf";
  }
...
function calculateStraightDistance(
  ...
  const estimatedMinutes = Math.max(
    1,
    Math.round((km / averageUrbanSpeedKmH) * 60)
  );
...
function buildRouteMapSrcDoc({ origin, destination, routeCoordinates }) {
  ...
  const safeCoordinates = Array.isArray(routeCoordinates)
    ? routeCoordinates
    : [];
...
<body>
  ...
  <script>
    const origin = ${JSON.stringify(safeOrigin)};
    const destination = ${JSON.stringify(safeDestination)};
    const routeCoordinates = ${JSON.stringify(safeCoordinates)};
    ...
    const routeLatLngs = routeCoordinates.map((item) => [item[1], item[0]]);

    const startMarker = L.marker(originLatLng)
      .addTo(map)
      .bindPopup("Staff current location");

    const endMarker = L.marker(destinationLatLng)
      .addTo(map)
      .bindPopup("Victim reported location");

    let bounds = L.latLngBounds([originLatLng, destinationLatLng]);
